	...
	public static class TriangleWalker
	{
		...
		public static TriangleCollection GetTrianglesFromGeometryShape(this NiTriShapeData shapeData, Matrix transformationMatrix)
		{
            return new TriangleCollection
            {
            	...
            	#else
            	// Unity Matrix4x4 API
            	Vertices = shapeData.Vertices.Select(vert => ((Matrix4x4)transformationMatrix).MultiplyPoint3x4(vert)).ToArray(),
            	#endif
		...
		public static TriangleCollection GetTrianglesFromGeometryStrips(this NiTriStripsData stripsData, Matrix transformationMatrix)
		{
            ...
            return new TriangleCollection
            {
            	...
            	#else
            	// Unity Matrix4x4 API
            	Vertices = stripsData.Vertices.Select(vert => ((Matrix4x4)transformationMatrix).MultiplyPoint3x4(vert)).ToArray(),
            	#endif
		...
		public static Matrix GetWorldMatrixFromNode(this NiAVObject leaf)
		{
			...
			while (current != null)
			{
				...
				var translate = Matrix4x4.Translate(new Vector3(current.Translation.x, current.Translation.y, current.Translation.z));
			...
			return tris.Vertices.Select((vert, ind) =>
			                            ...
			                            .Aggregate(new Vector3(0, 0, 0),
			                                       (v, t) => {
			                                       	...
			                                       	return result;
			                                       	#else
			                                       	// Unity Vector3 API - use operators
			                                       	Vector3 AB = tris.Vertices[t.B] - tris.Vertices[t.A];
			                                       	Vector3 AC = tris.Vertices[t.C] - tris.Vertices[t.A];
			                                       	Vector3 cross = Vector3.Cross(AB, AC);
			                                       	Vector3 crossnorms = cross.normalized;
			                                       	return v + crossnorms;
			                                       	#endif
			                                       }
			                                       , a => { 
			                                       	#if OpenTK || SharpDX || MonoGame
			                                       	Vector3 n; Vector3.Normalize(ref a, out n); return n;
			                                       	#else
			                                       	// Unity Vector3 API
			                                       	return a.normalized;
			                                       	#endif
